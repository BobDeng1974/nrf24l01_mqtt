...
#define MQTT_CLIENTS_LIST_FULL		0xFF
...
typedef int (*mqtt_send_t)(void* socket_info, const void* buf, unsigned int count);
...
typedef struct {
	void* socket_info;
	mqtt_send_t send;
	mqtt_client_handle_t clients[MAX_CLIENTS];
} mqtt_broker_t;




/** Initialize the information to connect to the client.
 * @param client Data structure that contains the connection information with the client.
...
/** Enable the authentication to connect to the client.
 * @param client Data structure that contains the connection information with the client.
